...
export default function SubmitButton() {
  ...
  const { addWordToList, clearWord, toggleAlreadyFound, toggleIncorrectWord } =
    gameStore()
  const isPressed = useKeyPress('Enter', () => handleSubmit())

  const { isPending, mutate } = useMutation({
    mutationFn: (w: string) => isValidWord(w),
    onSuccess: isValid => {
      if (isValid) {
        addWordToList()
      } else {
        clearWord()
        toggleIncorrectWord(true)
      }
    },
  })
  ...
  const handleSubmit = () => {
    ...
    if (alreadyUsed || word.length < 3) {
      clearWord()
      if (alreadyUsed) toggleAlreadyFound(true)
      if (word.length < 3) toggleIncorrectWord(true)
      return
    }
...
const LoadingIcon = () => (
  <div role="status">
    <svg
      aria-hidden="true"
      className="mr-2 h-6 w-6 animate-spin text-[--clr-activate]"
      viewBox="0 0 100 101"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
